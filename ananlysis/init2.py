...
2. Data profiling & outlier detection (Group A / Section 6)
3. Actuator & spray effectiveness engine (Group B & Group D)
4. Command-Line Interface (CLI)
5. Embedded Unit Tests (--test flag)
"""
...
def compute_weekly_report(df: pd.DataFrame, week_start: str, config: dict = None) -> dict:
    ...
    # 4. Data Health & Coverage Metrics (Group C)
    ...
    # 5. Environment Metrics & Outliers (Group A & Section 6)
    ...
    # Outlier Detection via Tukey's 1.5x IQR Fence
    t_lower_fence = t_q25 - 1.5 * t_iqr
    t_upper_fence = t_q75 + 1.5 * t_iqr
    t_outliers_count = int(((week_df["temperature"] < t_lower_fence) | (week_df["temperature"] > t_upper_fence)).sum())

    t_max_idx = week_df["temperature"].idxmax()
    t_max_time = week_df.loc[t_max_idx, "time_dt"].isoformat() if pd.notna(t_max_idx) else None

    # Time & Longest Spell Above Temperature Threshold
    above_thresh_df = week_df[week_df["temperature"] > temp_threshold]
    time_above_sec = above_thresh_df["duration_sec"].sum()
    pct_above = (time_above_sec / total_logged_sec) * 100 if total_logged_sec > 0 else 0.0

    # Calculate Longest Continuous Spell Above Threshold (incorporating outage gaps)
    week_df["above_thresh"] = week_df["temperature"] > temp_threshold
    week_df["heat_spell_id"] = ((week_df["above_thresh"] != week_df["above_thresh"].shift()) |
                                (week_df["duration_sec"] > gap_threshold)).cumsum()

    heat_spells = week_df[week_df["above_thresh"]].groupby("heat_spell_id").agg(
        spell_duration=("duration_sec", "sum")
    )
    longest_heat_spell_min = float(heat_spells["spell_duration"].max() / 60.0) if not heat_spells.empty else 0.0

    # Humidity Statistics & Outliers
    ...
    h_lower_fence = h_q25 - 1.5 * h_iqr
    h_upper_fence = h_q75 + 1.5 * h_iqr
    h_outliers_count = int(((week_df["humidity"] < h_lower_fence) | (week_df["humidity"] > h_upper_fence)).sum())
    ...
    # Spray Event Duration Analysis (Gap-Aware)
    week_df["event_id"] = ((week_df["relay3"] != week_df["relay3"].shift()) |
                           (week_df["duration_sec"] > gap_threshold)).cumsum()
    ...
    # 7. Group D — Spray Cooling Effectiveness Analysis
    spray_starts = week_df[(week_df["relay3"] == 1) & (week_df["relay3"].shift(1, fill_value=0) == 0)]
    temp_drops = []

    for _, start_row in spray_starts.iterrows():
        t_start = start_row["time_dt"]
        t_before = start_row["temperature"]
        t_window_end = t_start + pd.Timedelta(minutes=10)

        post_spray = week_df[(week_df["time_dt"] >= t_start) & (week_df["time_dt"] <= t_window_end)]
        if not post_spray.empty:
            t_min_post = post_spray["temperature"].min()
            temp_drops.append(t_before - t_min_post)

    if temp_drops:
        eff_med = float(np.median(temp_drops))
        eff_q25 = float(np.percentile(temp_drops, 25))
        eff_q75 = float(np.percentile(temp_drops, 75))
        eff_iqr = float(eff_q75 - eff_q25)
    else:
        eff_med, eff_iqr = 0.0, 0.0

    # 8. Output Payload JSON Structure
    report = {
        "week_start": week_start,
        "week_end": (start_dt + pd.Timedelta(days=6)).strftime("%Y-%m-%d"),
        "coverage": {
        ...
        "temperature_c": {
            ...
            "longest_spell_above_threshold_minutes": round(longest_heat_spell_min, 1),
            "outliers_count_iqr_fence": t_outliers_count
        },
        "humidity_percent": {
            "Median": round(h_med, 2),
            "IQR": round(h_iqr, 2),
            "outliers_count_iqr_fence": h_outliers_count
        },
        "actuators": {
            ...
            "total_spray_minutes": round(r3_total_minutes, 2),
        ...
        "effectiveness_group_d": {
            "median_temp_drop_c_10min_post_spray": round(eff_med, 2),
            "temp_drop_iqr": round(eff_iqr, 2)
        },
        "warnings": warnings
    }
